...
from models import multimodal_arquitecture_concatenate
...
for ii in range(len(parameters)):
    ...
    for jj in range(1,6):
        ...
        #%%
        early_stopping = EarlyStopping(patience=15, verbose=True, name="checkpoints_va+av/checkpoint"+str(fold)+".pt")
        model_multimodal = multimodal_arquitecture_concatenate.MultiModalNet(num_classes=2, drop=parameters[ii][0], num_heads=1)
        ...
        model_multimodal.load_state_dict(torch.load("checkpoints_va+av/checkpoint"+str(fold)+".pt"))
        model_multimodal.eval()    
        ...
        ut.plot_results(loss_aux, test_loss_aux, 0, fold,'./results_va+av/')
    ...
    ut.save_results('./results_va+av/results.csv', 'acc', ii, acc_total)
    ut.save_results('./results_va+av/results.csv', 'acc_bal', ii, acc_bal_total)
    ut.save_results('./results_va+av/results.csv', 'sensi', ii, sensi_total)
    ut.save_results('./results_va+av/results.csv', 'speci', ii, speci_total)
    ut.save_results('./results_va+av/results.csv', 'f1-sco', ii, f1_total)
    ut.save_results('./results_va+av/results.csv', 'AUC', ii, auc_total)
    
    ut.save_results('./results_va+av/results.csv', 'labels', ii, list(np.hstack(y_final)))
    ut.save_results('./results_va+av/results.csv', 'predict', ii, list(np.hstack(z_final)))
    ut.save_results('./results_va+av/results.csv', 'scores', ii, list(np.hstack(y_scores_final)))
    ut.save_results('./results_va+av/results.csv', 'id_test', ii, list(np.hstack(id_test_final)))
    ut.save_results('./results_va+av/results.csv', 'parameters', ii, parameters[ii])
